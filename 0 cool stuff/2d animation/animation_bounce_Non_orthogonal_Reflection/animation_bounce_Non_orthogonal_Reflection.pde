...
float ellipseX, ellipseY, ellipseRadius = 8;
...
directionX = random(.1, .99);
directionY = random(.1, .99);
...
if (dist(ellipseX, ellipseY, xCoords[i], 
yCoords[i]) < ellipseRadius){
// calculate dot product of incident vector and base top normal
float dot = incidenceVectorX*normalX + 
incidenceVectorY*normalY;
...
directionY *= -1;
// randomize base top
baseY1 = random(height-300, height);
baseY2 = random(height-300, height);
}
}