...
abstract class VizObj{
  ...
  public void setRot(PVector rot){
    ...
    for (int i=0; i<vecs.length; i++){
      // x-axis rotation
      tempVecs[i] = new PVector (vecs[i].x, cx*vecs[i].y -  
        sx*vecs[i].z, sx*vecs[i].y + cx*vecs[i].z);
      // y-axis rotation
      vecs[i] = new PVector (sy*tempVecs[i].z + cy*tempVecs[i].x,  
      tempVecs[i].y, cy*tempVecs[i].z - sy*tempVecs[i].x);
      // z-axis rotation
      tempVecs[i] = new PVector (cz*vecs[i].x - sz*vecs[i].y,  
      sz*vecs[i].x + cz*vecs[i].y, vecs[i].z);
    }
