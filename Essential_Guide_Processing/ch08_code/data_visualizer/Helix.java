...
public class Helix extends VizObj{
  ...
  // constructor
  public Helix(PApplet p, float w, float h, int rots, int helixDetail, float tubeRadius, int tubeDetail){
    super(p, w, h);
    this.rots = rots;
    this.helixDetail = helixDetail;
  ...
  protected void init(){
    ...
    for (int i=0; i<path.length; i++){
      path[i] = new PVector(p.sin(helixTheta)*w/2.0f, -h/2.0f+offset*i, p.cos(helixTheta)*w/2.0f);
      float tubeTheta = 0.0f;
      for (int j=0; j<tubeDetail; j++){
        // create cross-sections around helix, with uncorrected y-rotation
        PVector temp = new PVector(0, p.cos(tubeTheta)*tubeRadius, p.sin(tubeTheta)*tubeRadius);
        // correct for y rotation of cross-section.
        PVector fixed = new PVector(p.sin(helixTheta)*temp.z - p.cos(helixTheta)*temp.x, temp.y, p.cos(helixTheta)*temp.z + p.sin(helixTheta)*temp.x);
        // populate vecs array with cross-section vertices, at Path vertex positions.
        vecs[tubeDetail*i + j] = new PVector(path[i].x + fixed.x, path[i].y + fixed.y, path[i].z + fixed.z);
      ...
      for (int i=1; i<path.length; i++){
        for (int j=1; j<tubeDetail; j++){
          if (j == tubeDetail-1){
            ...
            p.vertex(finalVecs[tubeDetail*(i-1)+(j-1)].x, finalVecs[tubeDetail*(i-1)+(j-1)].y, finalVecs[tubeDetail*(i-1)+(j-1)].z);
            p.fill(arrR[i], arrG[i], arrB[i]);
            p.vertex(finalVecs[tubeDetail*(i)+(j-1)].x, finalVecs[tubeDetail*(i)+(j-1)].y, finalVecs[tubeDetail*(i)+(j-1)].z);
            p.fill(arrR[i], arrG[i], arrB[i]);
            p.vertex(finalVecs[tubeDetail*(i)+(0)].x, finalVecs[tubeDetail*(i)+(0)].y, finalVecs[tubeDetail*(i)+(0)].z);
            p.fill(arrR[i-1], arrG[i-1], arrB[i-1]);
            p.vertex(finalVecs[tubeDetail*(i-1)+(0)].x, finalVecs[tubeDetail*(i-1)+(0)].y, finalVecs[tubeDetail*(i-1)+(0)].z);
          } 
          p.fill(arrR[i-1], arrG[i-1], arrB[i-1]);
          p.vertex(finalVecs[tubeDetail*(i-1)+(j-1)].x, finalVecs[tubeDetail*(i-1)+(j-1)].y, finalVecs[tubeDetail*(i-1)+(j-1)].z);
          p.fill(arrR[i], arrG[i], arrB[i]);
          p.vertex(finalVecs[tubeDetail*(i)+(j-1)].x, finalVecs[tubeDetail*(i)+(j-1)].y, finalVecs[tubeDetail*(i)+(j-1)].z);
          p.fill(arrR[i], arrG[i], arrB[i]);
          p.vertex(finalVecs[tubeDetail*(i)+(j)].x, finalVecs[tubeDetail*(i)+(j)].y, finalVecs[tubeDetail*(i)+(j)].z);
          p.fill(arrR[i-1], arrG[i-1], arrB[i-1]);
          p.vertex(finalVecs[tubeDetail*(i-1)+(j)].x, finalVecs[tubeDetail*(i-1)+(j)].y, finalVecs[tubeDetail*(i-1)+(j)].z);
        }
      } 
      p.endShape();
      // gradient end cap
  ...
  public void create(){
    ...
    for (int i=1; i<path.length; i++){
      for (int j=1; j<tubeDetail; j++){
        if (j == tubeDetail-1){
          p.vertex(vecs[tubeDetail*(i-1)+(j-1)].x, vecs[tubeDetail*(i-1)+(j-1)].y, vecs[tubeDetail*(i-1)+(j-1)].z);
          p.vertex(vecs[tubeDetail*(i)+(j-1)].x, vecs[tubeDetail*(i)+(j-1)].y, vecs[tubeDetail*(i)+(j-1)].z);
          p.vertex(vecs[tubeDetail*(i)+(0)].x, vecs[tubeDetail*(i)+(0)].y, vecs[tubeDetail*(i)+(0)].z);
          p.vertex(vecs[tubeDetail*(i-1)+(0)].x, vecs[tubeDetail*(i-1)+(0)].y, vecs[tubeDetail*(i-1)+(0)].z);
        } 
        p.vertex(vecs[tubeDetail*(i-1)+(j-1)].x, vecs[tubeDetail*(i-1)+(j-1)].y, vecs[tubeDetail*(i-1)+(j-1)].z);
        p.vertex(vecs[tubeDetail*(i)+(j-1)].x, vecs[tubeDetail*(i)+(j-1)].y, vecs[tubeDetail*(i)+(j-1)].z);
        p.vertex(vecs[tubeDetail*(i)+(j)].x, vecs[tubeDetail*(i)+(j)].y, vecs[tubeDetail*(i)+(j)].z);
        p.vertex(vecs[tubeDetail*(i-1)+(j)].x, vecs[tubeDetail*(i-1)+(j)].y, vecs[tubeDetail*(i-1)+(j)].z);
      }
    }
    p.endShape();
